...
import { Search, MapPin, DollarSign, Calendar, Copy, ChevronRight, Check, X, Sparkles, Plus, Loader2 } from "lucide-react";
...
export default function JobsBoard({
  ...
  const [location, setLocation] = useState("Beirut, Lebanon (Hybrid)");
  ...
  const filteredJobs = jobs.filter((job) => {
    ...
    const matchSkills = job.skills.some((sk) => sk.toLowerCase().includes(query));
  ...
  const handleJobSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCompany("");
      setType("Full-Time");
      setLocation("Beirut, Lebanon (Hybrid)");
      setSalary("");
      setDescription("");
      setSkillInput("");
      setLogo("💼");
  ...
  const handleGenerateAI = async (e: React.FormEvent) => {
    ...
    if (!applicantName.trim()) {
      setAiError("Applicant Name is required.");
      return;
    }
    ...
    try {
      const res = await fetch("/api/cover-letter", {
        ...
        body: JSON.stringify({
          ...
          skillsSummary: skillsSummary || selectedJob.skills.join(", "),
        }),
      });
      ...
      if (res.ok) {
        ...
        setAiError(data.error || "Failed to generate pitch letter.");
      }
    } catch (err: any) {
      setAiError(err.message || "Server issue generating pitch letter.");
    } finally {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6" id="jobs_board_container">
      {/* LEFT: Jobs feed rows */}
      <div className={`${selectedJob ? "lg:col-span-6" : "lg:col-span-12"} space-y-4`}>
        <div className="bg-white p-4 border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-[#FF6600] text-white border-2 border-black font-black uppercase text-xs px-4 py-2 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[0.5px] hover:translate-y-[0.5px] cursor-pointer hover:shadow-none transition-all"
          >
        ...
        <div className="space-y-3.5" id="jobs_list_grid">
          {filteredJobs.length === 0 ? (
            <div className="bg-white border-2 border-black p-12 text-center text-gray-500 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]" id="empty_jobs_prompt">
              <p className="font-extrabold text-lg uppercase font-display text-black">No job listings found</p>
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-3.5">
                    ...
                    <div>
                      <h3 className="font-display font-black text-black leading-tight uppercase tracking-tight text-base hover:text-[#FF6600]">
                        {job.title}
                      </h3>
                      ...
                      <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-gray-500 font-mono mt-1 w-full">
                        <span className="flex items-center gap-0.5 font-bold text-black bg-[#F6F6EF] border border-black px-1.5 py-0.2">
                          <MapPin className="w-3 h-3 text-[#FF6600]" />
                          <span>{job.location.toUpperCase()}</span>
                        </span>
                        <span className="text-[#FF6600] font-black">{job.salary.toUpperCase()}</span>
                        <span>•</span>
      ...
      {/* RIGHT: Jobs Detail and Cover Letter Generator */}
      {selectedJob && (
        <div className="col-span-12 lg:col-span-6 animate-fade-in" id="job_details_panel">
          <div className="bg-white border-2 border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] overflow-hidden flex flex-col text-black rounded-none">
            {/* Header */}
            <div className="bg-[#FFF9E6] p-4 border-b-2 border-black flex items-start justify-between">
              <div className="flex gap-3 items-center">
                ...
                <div>
                  <h3 className="font-display font-black text-black leading-tight uppercase tracking-tight text-base">{selectedJob.title}</h3>
                  <p className="text-xs text-gray-600 font-mono font-bold uppercase tracking-tight">{selectedJob.company} • {selectedJob.location.toUpperCase()}</p>
                </div>
              </div>
              <button
                id="close_job_details"
                onClick={() => setSelectedJob(null)}
                className="text-black hover:text-[#FF6600] p-1.5 border-2 border-black bg-white shadow-[1.5px_1.5px_0px_0px_rgba(0,0,0,1)] cursor-pointer"
              >
            ...
            <div className="p-5 space-y-5 overflow-y-auto max-h-[75vh]">
              ...
              <div>
                ...
                <p className="text-xs text-black leading-relaxed bg-[#F6F6EF] p-3.5 border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] whitespace-pre-line select-text">
              ...
              <div className="border-t-2 border-dashed border-black pt-5 space-y-4">
                <div className="flex items-center gap-2">
                  <span className="p-1.5 bg-[#FFF9E6] text-[#FF6600] border-2 border-black shadow-[1.5px_1.5px_0px_0px_rgba(0,0,0,1)]">
                    <Sparkles className="w-4 h-4 fill-[#FF6600]" />
                  </span>
                  <div>
                    <h4 className="font-display font-black text-sm text-black uppercase tracking-tight">AI Instant Pitch Builder</h4>
                ...
                {!generatedLetter ? (
                  <form onSubmit={handleGenerateAI} className="space-y-3 bg-[#FFF9E6] p-4 border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)]" id="ai_letter_form">
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        className="w-full text-xs bg-white border-2 border-black p-2 outline-none text-black focus:shadow-[2px_2px_0px_0px_rgba(255,102,0,1)]"
                      />
                    </div>
                    ...
                    <button
                      ...
                      className="w-full bg-[#FF6600] text-white border-2 border-black font-black uppercase text-xs py-2 px-3 shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:translate-x-[1px] active:translate-y-[1px] cursor-pointer transition-all flex items-center justify-center gap-1.5"
                    >
                      {isGenerating ? (
                        <>
                  ...
                  <div className="space-y-3 bg-emerald-50 border-2 border-[#10B981] p-4 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]" id="ai_letter_response_pane">
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-mono font-bold uppercase text-emerald-950 bg-emerald-100 border border-emerald-300 px-2.5 py-0.5">
                        AI Draft Completed
                      </span>
                      <div className="flex items-center gap-2">
                        <button
                        ...
                        <button
                          ...
                          className="text-[11px] text-black hover:text-[#FF6600] font-black underline font-mono uppercase cursor-pointer"
                        >
                          Retry
                        </button>
                      </div>
                    ...
                    <div className="bg-white p-4 border border-black leading-relaxed text-xs text-black whitespace-pre-wrap font-sans max-h-[300px] overflow-y-auto select-all">
                      {generatedLetter}
                    </div>
                  </div>
                )}
          ...
          <div className="bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] overflow-hidden max-w-xl w-full rounded-none text-black">
            <div className="bg-[#FFF9E6] p-4 border-b-2 border-black flex items-center justify-between">
              ...
              <button
                ...
                className="text-black hover:text-[#FF6600] p-1 border-2 border-black bg-white transition hover:translate-x-[0.5px] hover:translate-y-[0.5px] font-black cursor-pointer"
              >
                <X className="w-4.5 h-4.5" />
              </button>
            </div>
            ...
            <form onSubmit={handleJobSubmit} className="p-5 space-y-4" id="add_job_form">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full text-xs bg-white border-2 border-black p-2 outline-none text-black font-mono shadow-[1.5px_1.5px_0px_0px_rgba(0,0,0,1)] focus:shadow-[2px_2px_0px_0px_rgba(255,102,0,1)]"
                  />
                </div>
                <div>
                  <label className="block text-xs font-mono font-bold text-black uppercase tracking-wider mb-1">
                  ...
                  <input
                    ...
                    type="text"
                    required
                    placeholder="e.g. Toters, PursePay"
                    value={company}
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  required
                  placeholder="Elaborate key roles, framework details, and fresh payroll clearing index..."
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  className="w-full text-xs bg-white border-2 border-black p-2 outline-none text-black shadow-[1.5px_1.5px_0px_0px_rgba(0,0,0,1)] focus:shadow-[2px_2px_0px_0px_rgba(255,102,0,1)]"
                />
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full text-xs bg-[#FFF9E6] border-2 border-black p-2 outline-none text-black font-mono"
                  />
                </div>
              </div>
